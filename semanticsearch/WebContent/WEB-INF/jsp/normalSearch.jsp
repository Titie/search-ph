...
Tìm kiếm theo từ khóa
	<div class="formSearch">
		<s:form id="searchForm" action="normalSearch"  method="GET" accept-charset="UTF-8">
			<table>
				<tr>
					<td >
	...
	<div class="content"> 
		...
		<ul class="listStats">
			<c:forEach items="${retrievals}" var="document">
				<li>
					<a href="${contextPath}/normalDetails?documentId=${document.id}" style="margin-bottom: 0px; margin-top: 10px;float:left;" target="_blank">
		...
		<c:if test="${pageNo != null}">
		          ...
		          <c:if test="${pageIndex >= 10}">
		          	<li class="previous-off">
		          		<a href="${contextPath}/normalSearch?q=${q}&isSubmit=true&pageIndex=1"> «Trước </a>
		          	</li>
		          </c:if>
					<c:forEach var="page" begin="${pageFirstLoop}" end="${pageEndLoop}">
						<c:if test="${page ne pageIndex}">
						 	<li><a href="${contextPath}/normalSearch?q=${q}&isSubmit=true&pageIndex=${page}">${page}</a></li>
						</c:if>
				   ...
				   <c:if test="${(pageIndex + 4 )< pageNo}">
		          	<li class="next"><a href="${contextPath}/normalSearch?q=${q}&isSubmit=true&pageIndex=${pageNo}">Sau »</a></li>
		          </c:if>
				</div>
		</c:if>
		</ul>
